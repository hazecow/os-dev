...
uint32_t lapic_read(uint64_t offset) {
  volatile uint32_t *reg_vaddr =
      (volatile uint32_t *)(g_lapic_base_vaddr + offset);
  return *reg_vaddr;
}
...
static void spurious_handler(interrupt_frame_t *frame) { (void)frame; }
...
static void lvt_timer_handler(interrupt_frame_t *frame) {
  (void)frame;
  // イベントキューに push
  // timeout は後ほど正しく計算できるようにする
  event_message_t msg = {.type = TIMER_TIMEOUT,
                         .arg = {.timer = {.timeout = 0, .value = 0}}};
...
void lapic_init(uint64_t *pml4) {
  ...
  vmm_map(pml4, (uint64_t)paddr_to_vaddr(g_lapic_base_paddr),
          g_lapic_base_paddr,
...
void ioapic_redirect(uint8_t irq, uint8_t vector) {
  ...
  uint8_t trigger_mode =
      iso_entry ? (iso_entry->flags.trigger_mode & 0b10) >> 1 : 0;
  ...
  // 下位 32 bit
  ioapic_write((uint64_t)paddr_to_vaddr(ioapic_entry->io_apic_paddr),
               0x10 + 2 * pin,
               (0 << 16) | (trigger_mode << 15) | (polarity << 13) | vector);
  // 上位 32 bit
  ioapic_write((uint64_t)paddr_to_vaddr(ioapic_entry->io_apic_paddr),
               0x11 + 2 * pin, (uint32_t)destination << 24);
}
...
void lapic_send_eoi(void) { lapic_write(EOI_OFFSET, 0); }
